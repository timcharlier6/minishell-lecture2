/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   file_io.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ticharli <ticharli@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/17 17:51:46 by mcombeau          #+#    #+#             */
/*   Updated: 2025/12/16 19:24:58 by ticharli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

/* restore_io:
*	Restores the original standard input and standard output
*	to their original fds of 0 and 1. Used to clear the input/output
*	fds after execution, in preparation for the next set of user commands.
*	Returns 1 if the duplication was successful, 0 if not.
*/
bool	restore_io(t_io_fds *io)
{
	if (!io)
		return (true);
	if (io->stdin_backup != -1)
	{
		if (dup2(io->stdin_backup, STDIN_FILENO) == -1)
			return (errmsg_cmd("dup2", "stdin restore", strerror(errno), false));
		close(io->stdin_backup);
		io->stdin_backup = -1;
	}
	if (io->stdout_backup != -1)
	{
		if (dup2(io->stdout_backup, STDOUT_FILENO) == -1)
			return (errmsg_cmd("dup2", "stdout restore", strerror(errno), false));
		close(io->stdout_backup);
		io->stdout_backup = -1;
	}
	return (true);
}

/* redirect_io:
*	Duplicates the input and output fds to the standard input and output.
*	Backs up the standard input and output before replacing them in order
*	to restore them after execution.
*	Returns 1 for success, 0 in case of error.
*/
bool	redirect_io(t_io_fds *io)
{
	if (!io)
		return (true);
	io->stdin_backup = dup(STDIN_FILENO);
	if (io->stdin_backup == -1)
		return (errmsg_cmd("dup", "stdin backup", strerror(errno), false));
	io->stdout_backup = dup(STDOUT_FILENO);
	if (io->stdout_backup == -1)
	{
		close(io->stdin_backup);
		io->stdin_backup = -1;
		return (errmsg_cmd("dup", "stdout backup", strerror(errno), false));
	}
	if (io->fd_in != -1)
	{
		if (dup2(io->fd_in, STDIN_FILENO) == -1)
			return (errmsg_cmd("dup2", io->infile, strerror(errno), false));
	}
	if (io->fd_out != -1)
	{
		if (dup2(io->fd_out, STDOUT_FILENO) == -1)
			return (errmsg_cmd("dup2", io->outfile, strerror(errno), false));
	}
	return (true);
}

/* check_infile_outfile:
*	Checks if the infile and outfile are set correctly.
*	Returns 1 on success, 0 on failure.
*/
bool	check_infile_outfile(t_io_fds *io)
{
	if (!io || (!io->infile && !io->outfile))
		return (true);
	if ((io->infile && io->fd_in == -1)
		|| (io->outfile && io->fd_out == -1))
		return (false);
	return (true);
}
